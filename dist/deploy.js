...
function load() {
    const selectList = document.getElementById("selecao");
    selectList.selectedIndex = 0;
    selectList.focus();
    const sectionView = document.getElementById("dataViewSection");
...
function listData(lista, selectList) {
...
function viewData() {
    const selectList = document.getElementById("selecao");
    const index = Number(selectList.value);
    const viewDiv = document.getElementById("dataViewDiv");
    const sectionView = document.getElementById("dataViewSection");
    if (index) {
        const giant = lista[index - 1];
...
function editValue(nameRadiogroup, index, viewDiv) {
    const giant = lista[index - 1];
    const inputBox = document.createElement("input");
    inputBox.type = "text";
    inputBox.id = "newValue";
    const labelInputBox = document.createElement("label");
    let editNome = document.getElementsByName(nameRadiogroup)[0];
    let attributeToEdit;
    if (editNome.checked) {
...
function excludeListItem(indexSelect, viewDiv) {
    const indexItemList = indexSelect - 1;
    const penultItem = lista.length - 1;
    if (window.confirm('Deseja continuar?')) {
        for (let i = indexItemList; i < penultItem; ++i) {
            lista[i] = lista[i + 1];
        }
        const selectList = document.getElementById("selecao");
        const lastOption = lista.length--;
